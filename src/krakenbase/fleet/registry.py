"""In-memory fleet registry for secondary monitor nodes."""
...
class FleetRegistry:
    """Track secondary nodes by heartbeat and task assignment."""

    def __init__(self, offline_after_s: float = 90.0):
    ...
    def _refresh_offline(self) -> None:
        ...
        for nid, node in list(self._nodes.items()):
            if node.last_seen < cutoff and node.status != SecondaryNodeStatus.OFFLINE:
                self._nodes[nid] = node.model_copy(update={"status": SecondaryNodeStatus.OFFLINE})
