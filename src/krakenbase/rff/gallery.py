...
class Gallery:
    ...
    def touch(self, uid: str) -> None:
        self._db.execute(
            "UPDATE emitters SET count=count+1, last_seen=? WHERE emitter_uid=?",
            (utcnow().isoformat(), uid),
        )
        self._db.commit()

    def label(self, uid: str, label: str) -> None:
        self._db.execute(
            "UPDATE emitters SET labeled=1, label=? WHERE emitter_uid=?",
            (label, uid),
        )
        self._db.commit()
    ...
    def ingest_sigmf(
        ...
        if file_rid and recipe_id and file_rid != recipe_id:
            return RffResult(
                freq_hz=int(freq_hz or info.get("freq_hz") or 0),
                sensor_id=sid,
                recipe_id=rid,
                disposition=RffDisposition.RECIPE_MISMATCH,
                notes=f"file recipe {file_rid} != {recipe_id}",
        ...
        if hit and hit.score >= self.match_thr:
            ...
            disp = RffDisposition.REPEAT if hit.count >= 1 else RffDisposition.RFF_MATCH
            if hit.count == 1:
                disp = RffDisposition.RFF_MATCH
            return RffResult(
                freq_hz=now_freq,
                sensor_id=sid,
                recipe_id=rid,
                disposition=disp,
                emitter_uid=hit.emitter_uid,
        ...
        if hit and hit.score >= self.new_thr:
            ...
            return RffResult(
                ...
                sensor_id=sid,
                recipe_id=rid,
                disposition=RffDisposition.RFF_MATCH,
                emitter_uid=hit.emitter_uid,
                score=round(hit.score, 4),
                source_event_id=source_event_id,
        ...
        return RffResult(
            ...
            sensor_id=sid,
            recipe_id=rid,
            disposition=RffDisposition.NEW,
            emitter_uid=uid,
